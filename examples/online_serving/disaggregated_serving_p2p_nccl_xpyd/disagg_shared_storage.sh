...
check_required_files() {
    local files=("disagg_shared_proxy.py")
...
cleanup() {
    ...
    pkill -9 -f "disagg_shared_proxy.py"
...
main() {
    ...
    for i in "${!PREFILL_GPU_ARRAY[@]}"; do
        ...
        --kv-transfer-config \
        "{\"kv_connector\":\"SharedStorageConnector\",\"kv_role\":\"kv_producer\",\"kv_buffer_size\":\"1e1\",\"kv_port\":\"$kv_port\"}" > prefill$((i+1)).log &
        PIDS+=($!)
    ...
    for i in "${!DECODE_GPU_ARRAY[@]}"; do
        ...
        --gpu-memory-utilization 0.45 \
        --kv-transfer-config \
        "{\"kv_connector\":\"SharedStorageConnector\",\"kv_role\":\"kv_consumer\",\"kv_buffer_size\":\"8e9\",\"kv_port\":\"$kv_port\"}" > decode$((i+1)).log &
        PIDS+=($!)
    ...
    echo "Starting proxy server on port $PROXY_PORT..."
    python3 disagg_shared_proxy.py --port $PROXY_PORT &
    PIDS+=($!)
