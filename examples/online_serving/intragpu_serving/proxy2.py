...
def main():
    ...
    async def process_request():
        ...
        try:
            ...
            #async for generator in forward_request(DECODE_SERVICE_URL, original_request_data):
            ...
            #generator = forward_request(PREFILL_SERVICE_URL, prefill_request)
            #response = await make_response(generator)
            # Execute decode stage and stream response
            #async for generator in forward_request(DECODE_SERVICE_URL, original_request_data):
                #continue
            #    async forward_request(PREFILL_SERVICE_URL, prefill_request)
                #    continue
            
            generator = forward_request(DECODE_SERVICE_URL, original_request_data)
            #await forward_request(PREFILL_SERVICE_URL, prefill_request)
            response = await make_response(generator)
    ...
    async def process_request_prefill():
        """Process a single request through prefill and decode stages"""
        try:
            original_request_data = await request.get_json()
    ...
    async def process_request_decode():
        """Process a single request through prefill and decode stages"""
        try:
            original_request_data = await request.get_json()
    ...
    async def handle_request():
        ...
        task1 = asyncio.create_task(process_request_prefill())
        ...
        if not await request_queue.enqueue(task2):
            return Response(
                response=b'{"error": "Decode Server busy, try again later"}',
                status=503,
                content_type="application/json",
            )
