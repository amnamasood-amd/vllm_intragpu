...
# export LD_LIBRARY_PATH=/workspace/clr/build/hipamd/lib:/opt/rocm/lib:$LD_LIBRARY_PATH
# MODEL=${MODEL:-/workspace/llama_3.1_70B}
...
main() {
    ...
    for i in "${!DECODE_GPU_ARRAY[@]}"; do
        ...
        VLLM_USE_V1=1 CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 VLLM_ROCM_USE_AITER=1 VLLM_USE_TRITON_FLASH_ATTN=0 vllm serve $MODEL \
        --host 0.0.0.0 \
        --port $port \
    ...
    # =============================================================================
    cd ../../../benchmarks/
    results_folder="/workspace/datasets"
    for qps in 20 10; do
        prefix="/workspace/hwcount/miscope/intragpu-128-lmsys-qps${qps}"
        python3 /workspace/miscope/miscope.py \
        --gpus=0,1,4,5 \
        --prefix="$prefix" \
        --cmd="vllm bench serve --port 10003 --seed $(date +%s) \
        --model $MODEL \
        --dataset-name custom \
        --request-rate $qps \
        --num-prompts 10000 \
        ...
        --result-filename intragpu-128-lmsys-qps${qps}.json"

    done   
    # for input_len in 4096; do
    #     for qps in 50 40 30 20 10 5 1; do
    #         prefix="/workspace/hwcount/miscope/intragpu-128-input_len${input_len}-qps${qps}"

    #         python3 /workspace/miscope/miscope.py \
    #             --gpus=0,1,2,3,4,5,6,7 \
    #             --prefix="$prefix" \
    #             --cmd="vllm bench serve --port 10002 --seed $(date +%s) \
    #             --model $MODEL \
    #             --dataset-name random \
    #             --num-prompts 1024 \
    ...
    #     # --save-result \
    #     #     --result-dir \"$results_folder\" \
    #     #     --result-filename intragpu-input-qps-${qps}.json"
    # done
