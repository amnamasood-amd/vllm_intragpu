...
from vllm.v1.core.sched.output import (CachedRequestData, NewRequestData, NewRequestDataPrefill,
                                       SchedulerOutput, SchedulerOutputPrefill)
...
class Scheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        if self.vllm_config.kv_transfer_config.kv_role=="kv_producer":
            self.prev_prefill_counter=0
        else:
            self.prev_prefill_counter=-1

        self.current_prefill_event_counter=0
        #self.current_prefill_event_status=False
        self.pending_prefill_requests: list[list[(int,str)]]=[] #pending request ids with their associated event ids
        ...
        self.request_scheduled_times={}
        self.prefill_scheduled_times={}
        ...
        if not preempted_reqs:
            while self.waiting and token_budget > 0:
                ...
                if request.num_computed_tokens != 0:
                    logger.info("New Request %s has num_computed_tokens %d. it should be 0", request.request_id,request.num_computed_tokens)
                ...
                new_blocks = self.allocated_block_ids[request.request_id]

                if new_blocks is None:
                    # The request cannot be scheduled.
                    logger.info("new_blocks is None for allocated request %s", request.request_id)
                    break
                ...
                self.request_scheduled_times[request.request_id]=scheduled_timestamp
                self.prefill_scheduled_times[request.request_id]=scheduled_timestamp
                if self.log_stats:
                ...
                #request.num_computed_tokens = num_computed_tokens #this should be 0 for prefill requests
                # Count the number of prefix cached tokens.
                if request.num_cached_tokens < 0:
                    request.num_cached_tokens = request.num_computed_tokens
                logger.info("request scheduled %s num_tokens %d token budget %d", request.request_id, num_new_tokens, token_budget)

        # Put back any skipped requests at the head of the waiting queue
        if skipped_waiting_requests:
            logger.info("skipped requests")
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        scheduled_new_reqs_prefill: list[Request] = []
        ...
        #             self.prefill_event_statuses = pickle.load(file)
        ...
        #just_finished_req_ids=[item[0] for item in just_finished]
        
        # logger.info("prefill_event_statuses")
        # print(self.prefill_event_statuses)
        prefill_finished=False #len(just_finished)>0
        new_prefill=False
        
        # For logging.
        scheduled_timestamp = time.time()
        logger.info("already running requests %d", len(self.running))
        #print([req.request_id for req in self.running])
        # First, schedule the RUNNING requests.
        req_index = 0
        while req_index < len(self.running) and token_budget > 0: 
            ...
            try:
                index_req_id= just_finished.index(request.request_id)
                ...
                #logger.info("in finished prefill block")
                #self.request_scheduled_times[request.request_id]=scheduled_timestamp
            except ValueError:
            ...
            while True:
                if num_new_tokens > 1: #its a running prefill request, do not allocate kvcacheblocks. it should also not preempt
                    new_blocks=None
                    can_schedule=False
                    scheduled_running_reqs_prefill.append(request)
                    assert request in self.running_prefill
                    break
                else: #decode request
                ...
                if new_blocks is None:
                    ...
                    logger.info("new_blocks should not be None")
            ...
            if num_new_tokens > 1:  #represents the already running prefill
                ...
                req_to_new_blocks[request.request_id] = new_blocks
                #if num_new_tokens>1:
                #    logger.info("just finished prefill request %s has num_new_tokens>1",request.request_id)
                num_scheduled_tokens[request.request_id] = num_new_tokens
        ...
        if not preempted_reqs:
            while self.waiting:
                ...
                if request.num_computed_tokens == 0: #for a non-preempted request it should go here #prefill or waiting for kv? in our case running prefill should have num_computed_tokens==0 while its running. do we need to run this code? 
                    ...
                    if self.connector is not None: #should return 0
                    ...
                    if num_computed_tokens>0:
                        logger.info("request %s has num_computed_tokens >0", request.request_id)
                ...
                if num_new_tokens > 1:
                    ...
                    logger.info("sending %s to prefill", request.request_id)
                if self.log_stats:
                    request.record_event(EngineCoreEventType.SCHEDULED,
                                         scheduled_timestamp)
                if request.status == RequestStatus.WAITING:
                    ...
                    logger.info("should not be in resumed req")
                    if num_new_tokens>1:
                        scheduled_resumed_reqs_prefill.append(request)
                    else:
                        scheduled_resumed_reqs.append(request)
                else:
                    raise RuntimeError(
                ...
                #num_scheduled_tokens[request.request_id] = num_new_tokens
                if num_new_tokens>1:
                    num_scheduled_tokens_prefill[request.request_id] = num_new_tokens
                else:
                    num_scheduled_tokens[request.request_id] = num_new_tokens
                #token_budget -= num_new_tokens
                request.status = RequestStatus.RUNNING
        ...
        #logger.info("running prefill requests")
        ...
        #         NewRequestDataPrefill.from_request(
        #             req, req_to_new_blocks[req.request_id].get_block_ids())
        #         for req in scheduled_new_reqs_prefill
        #     ]

        #     cached_reqs_data_prefill = self._make_cached_request_data(
        #         [], #scheduled_running_reqs_prefill, #the running prefill requests dont need to have cached data, we assume it will be done in one scheduling step
        #         scheduled_resumed_reqs_prefill,
        #         num_scheduled_tokens_prefill,
        #         scheduled_spec_decode_tokens,
        #         req_to_new_blocks,
        #     )
        #     logger.info("Cached req data prefill")
        #     print(cached_reqs_data_prefill)
            
        #     scheduler_output_prefill = SchedulerOutputPrefill(
        #         scheduled_new_reqs=new_reqs_data_prefill,
        #         scheduled_cached_reqs=cached_reqs_data_prefill,
        #         num_scheduled_tokens=num_scheduled_tokens_prefill,
        #         total_num_scheduled_tokens=sum(num_scheduled_tokens_prefill.values()),
        #         num_common_prefix_blocks=num_common_prefix_blocks,
        #         # finished_req_ids is an existing state in the scheduler,
        #         # instead of being newly scheduled in this step.
        #         # It contains the request IDs that are finished in between
    ...
    def _update_after_schedule(
        ...
        for req_id, num_scheduled_token in num_scheduled_tokens.items():
            ...
            #     queueing_delay=self.prefill_scheduled_times[req_id]-request.arrival_time
            #     prefill_arrival=self.request_scheduled_times[req_id]-self.prefill_scheduled_times[req_id]
            #     ttft=generated_timestamp-self.prefill_scheduled_times[req_id]
            #     logger.info("request %s prefill_arrival %f ttft %f queueing delay %f", req_id, prefill_arrival, ttft,queueing_delay)
            #TODO implement num_computed_tokens increase for prefill after decode instance knows it has finished. TODO in sscheule
    ...
    def prefill_update_from_output(
        self,
        scheduler_output: SchedulerOutput,
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            #assume prefill requests will be finished at this point
            stopped=True
        ...
        if stopped_requests:
            ...
            self.finished_req_ids_prefill+=stopped_requests
            logger.info("finished_req_ids_prefill")
            print(self.finished_req_ids_prefill)
    ...
    def update_from_output(
        ...
        #first update the prefill event status from modelrunneroutput
        #self.current_prefill_event_status=model_runner_output.current_event_status
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if stopped:
                ...
                #logger.info("output tokens for request %s, req_id")
                #print(request._output_token_ids)
    ...
    def add_request(self, request: Request) -> None:
        #logger.info("Adding request %s with prompt_tokens %d spec_tokens %d output_placeholders %d computed_tokens %d", request.request_id, request.num_prompt_tokens, request.num_tokens_with_spec, request.num_output_placeholders, request.num_computed_tokens)
